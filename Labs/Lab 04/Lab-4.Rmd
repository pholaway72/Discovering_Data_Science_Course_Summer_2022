...
**Answer:** (Student Response Here) The key difference is the control. In experimental design we have control and we see what happens, in observational we do not have control and we just observe what happens. 
...
**Answer:** (Student Response Here) Observational Study.
...
**Answer:** (Student Response Here) Experimental Design since Abhi is testing his pickup line to a control group of Google pickup line. Answers can vary, but should be Experimental design with valid justification.
...
**Answer:** (Student Response here) Answer may vary, but should be along the lines of...Blocking creates homogeneous groups and reduces the effects of "unwanted" variables. The response could also talk about how it increases control of the variables in the experiment. 
...
df <- read.csv("~/Classes/DPISu22/Data Sets/Data.csv", stringsAsFactors=TRUE)
```
...
**Answer:** (Student Response Here) Olivia's method is not random and can create skewed data. She also might be creating bias in her way.  
...
**Answer:** (Student Response Here) They should say Taylor because her sampling method is SRS which decreases bias. If they choose Olivia, most likely their reasoning is something to do with ease of splitting. The correct/best answer should be Taylor.  
...
**Answer:** (Student Response Here) There could be high variability or even bias because the sample size is quite small. Taking samples from small populations often leads to high variability and potentially even create bias.
